...
EZSP_SCHEMA = {
    ...
    vol.Optional(c.CONFIG_PACKET_BUFFER_COUNT.name, default=0xFF): cv_optional_int(
        min=1, max=255
    ),
    # The maximum number of router neighbors the stack can keep track of. A neighbor
    # is a node within radio range
    vol.Optional(c.CONFIG_NEIGHBOR_TABLE_SIZE.name): cv_optional_int(min=8, max=16),
    #
    # The maximum number of APS retried messages the stack can be transmitting at
    # any time
    vol.Optional(c.CONFIG_APS_UNICAST_MESSAGE_COUNT.name): cv_optional_int(
        min=0, max=255
    ),
    #
    # The maximum number of non-volatile bindings supported by the stack
    vol.Optional(c.CONFIG_BINDING_TABLE_SIZE.name): cv_optional_int(min=0, max=32),
    #
    ...
    vol.Optional(c.CONFIG_ADDRESS_TABLE_SIZE.name, default=16): cv_optional_int(min=0),
    #
    ...
    vol.Optional(c.CONFIG_ROUTE_TABLE_SIZE.name): cv_optional_int(min=0, max=255),
    #
    # The number of simultaneous route discoveries that a node will support
    vol.Optional(c.CONFIG_DISCOVERY_TABLE_SIZE.name): cv_optional_int(min=0, max=255),
    #
    # The size of the alarm broadcast buffer
    vol.Optional(c.CONFIG_BROADCAST_ALARM_DATA_SIZE.name): cv_optional_int(
        min=0, max=16
    ),
    #
    # The size of the unicast alarm buffers allocated for end device children
    vol.Optional(c.CONFIG_UNICAST_ALARM_DATA_SIZE.name): cv_optional_int(min=0, max=16),
    #
    # Specifies the stack profile
    vol.Optional(c.CONFIG_STACK_PROFILE.name, default=2): cv_optional_int(
        min=0, max=255
    ),
    #
    ...
    vol.Optional(c.CONFIG_SECURITY_LEVEL.name, default=5): cv_optional_int(
        min=0, max=5
    ),
    #
    # The maximum number of hops for a message
    vol.Optional(c.CONFIG_MAX_HOPS.name): cv_optional_int(min=0, max=30),
    #
    # The maximum number of end device children that a router will support
    vol.Optional(c.CONFIG_MAX_END_DEVICE_CHILDREN.name, default=32): cv_optional_int(
        min=0, max=32
    ),
    #
    ...
    vol.Optional(
        c.CONFIG_INDIRECT_TRANSMISSION_TIMEOUT.name, default=7680
    ...
    vol.Optional(c.CONFIG_END_DEVICE_POLL_TIMEOUT.name, default=60): cv_optional_int(
        min=0, max=255
    ),
    #
    ...
    # from its tables
    vol.Optional(c.CONFIG_MOBILE_NODE_POLL_TIMEOUT.name): cv_optional_int(min=0),
    #
    # The number of child table entries reserved for use only by mobile nodes
    vol.Optional(c.CONFIG_RESERVED_MOBILE_CHILD_ENTRIES.name): cv_optional_int(
        min=0, max=254
    ),
    #
    # Enables boost power mode and/or the alternate transmitter output
    vol.Optional(c.CONFIG_TX_POWER_MODE.name): cv_optional_int(min=0, max=3),
    #
    # 0: Allow this node to relay messages. 1: Prevent this node from relaying
    # messages
    vol.Optional(c.CONFIG_DISABLE_RELAY.name): cv_optional_int(min=0, max=1),
    #
    ...
    vol.Optional(
        c.CONFIG_TRUST_CENTER_ADDRESS_CACHE_SIZE.name, default=2
    ): cv_optional_int(min=0),
    #
    # The size of the source route table
    vol.Optional(c.CONFIG_SOURCE_ROUTE_TABLE_SIZE.name, default=16): cv_optional_int(
        min=0
    ),
    # The units used for timing out end devices on their parent
    vol.Optional(
        c.CONFIG_END_DEVICE_POLL_TIMEOUT_SHIFT.name, default=8
    ): cv_optional_int(min=0, max=10),
    #
    # The number of blocks of a fragmented message that can be sent in a single
    # window
    vol.Optional(c.CONFIG_FRAGMENT_WINDOW_SIZE.name): cv_optional_int(min=0, max=8),
    #
    # The time (ms) the stack will wait between sending blocks of a fragmented
    # message
    vol.Optional(c.CONFIG_FRAGMENT_DELAY_MS.name): cv_optional_int(min=0),
    #
    ...
    vol.Optional(c.CONFIG_KEY_TABLE_SIZE.name, default=4): cv_optional_int(min=0),
    #
    # The APS ACK timeout value (ms). The stack waits this amount of time between
    # resends of APS retried messages
    vol.Optional(c.CONFIG_APS_ACK_TIMEOUT.name): cv_optional_int(min=0),
    #
    ...
    vol.Optional(c.CONFIG_ACTIVE_SCAN_DURATION.name): cv_optional_int(min=0, max=6),
    #
    # The time (seoonds) the coordinator will wait for a second end device bind
    # request to arrive
    vol.Optional(c.CONFIG_END_DEVICE_BIND_TIMEOUT.name): cv_optional_int(min=1),
    #
    ...
    vol.Optional(
        c.CONFIG_PAN_ID_CONFLICT_REPORT_THRESHOLD.name, default=2
    ...
    vol.Optional(c.CONFIG_REQUEST_KEY_TIMEOUT.name): cv_optional_int(min=0, max=10),
    #
    ...
    vol.Optional(c.CONFIG_CERTIFICATE_TABLE_SIZE.name): cv_optional_int(min=0, max=1),
    #
    ...
    vol.Optional(c.CONFIG_BROADCAST_TABLE_SIZE.name): cv_optional_int(min=15, max=254),
    #
    # The size of the MAC filter list table
    vol.Optional(c.CONFIG_MAC_FILTER_TABLE_SIZE.name): cv_optional_int(min=0, max=254),
    #
    # The number of supported networks
    vol.Optional(c.CONFIG_SUPPORTED_NETWORKS.name, default=1): cv_optional_int(
        min=1, max=2
    ),
    #
    ...
    vol.Optional(c.CONFIG_SEND_MULTICASTS_TO_SLEEPY_ADDRESS.name): cv_optional_int(
        min=0, max=1
    ),
    #
    # ZLL group address initial configuration
    vol.Optional(c.CONFIG_ZLL_GROUP_ADDRESSES.name): cv_optional_int(min=0, max=255),
    #
    # ZLL RSSI threshold initial configuration
    vol.Optional(c.CONFIG_ZLL_RSSI_THRESHOLD.name): cv_optional_int(min=-128, max=127),
    #
    ...
    # least five
    vol.Optional(c.CONFIG_RF4CE_PAIRING_TABLE_SIZE.name): cv_optional_int(
        min=0, max=126
    ),
    #
    # The maximum number of outgoing RF4CE packets supported by the stack
    vol.Optional(
        c.CONFIG_RF4CE_PENDING_OUTGOING_PACKET_TABLE_SIZE.name
    ): cv_optional_int(min=0, max=16),
    #
    # Toggles the MTORR flow control in the stack. 0=false, 1=true
    vol.Optional(c.CONFIG_MTORR_FLOW_CONTROL.name): cv_optional_int(min=0, max=1),
    #
    # Deprecated
    # The amount of time a trust center will store a transient key
    # with which a device can use to join the network
    vol.Optional(c.CONFIG_TRANSIENT_KEY_TIMEOUT_S.name): cv_optional_int(
        min=0, max=65535
    ),
}
